...
interface CemeterySelectScreenProps {
  ...
  // Pulling the list down from the top reloads the cemeteries
...
export const CemeterySelectScreen: React.FC<CemeterySelectScreenProps> = ({
  ...
  // A pull also retries the phone's position when Nearby is waiting on it
  const handleRefresh = () => {
    if (nearbyNeedsLocation && locationStatus !== 'locating') onRetryLocation();
    return onRefresh();
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
      ...
      {/* Cemetery List, pulled down from the top to refresh */}
      <PullToRefresh onRefresh={handleRefresh} className="flex-1 p-4">
